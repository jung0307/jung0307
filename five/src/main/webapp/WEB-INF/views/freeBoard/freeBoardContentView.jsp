		...
		<c:when test="${not empty boardVO}">
			<table border="1" style="width:100%; border-collapse: collapse;" id="table_">
				<tr style="border-bottom: soild; border-bottom-color: #cccccc; border-bottom-width: 0.5px;">
					<td style="width:15%; padding: 10px; max-width: 15%;" class="td_header">${boardVO.boardID }</td>
					<td style="width:55%; padding: 10px; max-width: 55%;" class="td_header">${boardVO.boardTitle }</td>
					<td style="width:20%; padding: 10px; max-width: 20%;" class="td_header">${boardVO.boardDate }</td>
					<td style="width:10%; padding: 10px; max-width: 10%;" class="td_header">${boardVO.boardCount }</td>
				</tr>
				<tr>
...
<%-- 							<c:choose>
				...
				<tr style="border-bottom: soild; border-bottom-color: #cccccc; border-bottom-width: 0.5px;">
<!-- 					
						<td style="float: right; padding: 15px;" colspan="2"> 
...
function remove() {
	...
	var pwd = prompt('비밀번호를 입력하세요!');
	if(pwd != null){
		location.href="${contextPath}/freeBoard/removeBoardContent.do?boardPwdStr="+pwd+"&boardNum=${boardVO.boardNum}&role=${memberVO.role}";
	}
}
...
function removeComment(id,commentNum,role) {
	...
	var pwd = prompt('비밀번호를 입력하세요!');
		...
		if((pwd!=null && pwd != "") || (pwd != null && role == "admin")){
			var commentVO = {
		...
		jsonInfo.commentList.forEach((item,idx)=>{
			...
			if(item.commentHir == 0){
				...
				$("#table_").append(
						...
						+    "<p class='card-text' style='text-align: left; word-break:break-all;'>"+item.commentContent+"</p>"
						+"</div>"
						+"</td></tr>"
